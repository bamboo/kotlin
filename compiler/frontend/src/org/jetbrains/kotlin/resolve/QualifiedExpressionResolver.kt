...
class QualifiedExpressionResolver(val symbolUsageValidator: SymbolUsageValidator) {
    ...
    private fun processSingleImport(
        ...
        if (candidates.isNotEmpty()) {
            ...
            val classDescriptor = createSyntheticClassDescriptorForUnresolvedImport(moduleDescriptor, importDirective) ?: return null
            return SingleImportScope(aliasName, listOf(classDescriptor))
        }
    ...
    /* For kapt.
       In stubs mode, when the declaration signature contains type generated by Annotation Processing,
       we don't know anything yet about this generated type, thus this type is marked as unresolved, and error/NonExistentClass
       is written to the class bytecode.
       There's a workaround: add "import foo.Bar as Bar" import manually (where Bar is the class generated by APT),
       and resolve to the synthetic class descriptor created by this function, so in backend we can write 
       a specific fq name (foo/Bar) instead of NonExistentClass.
     */
    private fun createSyntheticClassDescriptorForUnresolvedImport(
            moduleDescriptor: ModuleDescriptor,
            importDirective: KtImportDirective
    ): ClassDescriptor? {
        if (importDirective.isAllUnder) return null
        val aliasName = importDirective.aliasName ?: return null
        val fqName = importDirective.importedFqName ?: return null
        val shortName = fqName.shortName()
        if (shortName.asString() != aliasName) return null

        val declarations = mutableListOf<ClassifierDescriptor>()
        val packageFragmentDescriptor = object : PackageFragmentDescriptorImpl(moduleDescriptor, fqName.parent()) {
            override fun getMemberScope() = object : MemberScopeImpl() {
                override fun getContributedClassifier(name: Name, location: LookupLocation) = declarations.firstOrNull { it.name == name }

                override fun getContributedDescriptors(
                        kindFilter: DescriptorKindFilter,
                        nameFilter: (Name) -> Boolean
                ): Collection<DeclarationDescriptor> {
                    if (!kindFilter.acceptsKinds(DescriptorKindFilter.NON_SINGLETON_CLASSIFIERS_MASK)) return emptyList()
                    return declarations
                }
        ...
        val superTypes = listOf(moduleDescriptor.builtIns.anyType)
        val classDescriptor = object : ClassDescriptorImpl(
                packageFragmentDescriptor, shortName, Modality.FINAL, ClassKind.CLASS, superTypes, SourceElement.NO_SOURCE) {}
        val primaryConstructor = ConstructorDescriptorImpl.create(classDescriptor, Annotations.EMPTY, true, SourceElement.NO_SOURCE)
        classDescriptor.initialize(MemberScope.Empty, emptySet(), primaryConstructor)

        declarations += classDescriptor
